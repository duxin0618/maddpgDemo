...
env = simple_adversary_v3.parallel_env(N=2, max_cycles=NUM_STEP, continuous_actions=True, dynamic_rescaling=False, render_mode="human")
...
for agent_i in range(NUM_AGENT):
    ...
    agent = Agent(memo_size=MEMORY_SIZE, obs_dim=obs_dim[agent_i], state_dim=state_dim, n_agent=NUM_AGENT, action_dim=action_dim[agent_i], lr_actor=LR_ACTOR, lr_critic=LR_CRITIC, fc1_dims=HIDDEN_SIZE, fc2_dims=HIDDEN_SIZE, gamma=GAMMA, tau=TAU, batch_size=BATCH_SIZE)
    agents.append(agent)

# 2 Main training loops
for episode_i in range(NUM_EPISODE):
    ...
    for step_i in range(NUM_STEP):
        ...
        # 2.2 Execute action
        multi_next_obs, multi_reward, multi_done, multi_truncation, multi_info = env.step(multi_actions)
        ...
        for agent_id in range(NUM_AGENT):
            ...
            batch_obses, batch_next_obses, batch_states, batch_next_states, batch_actions, batch_rewards, batch_dones = agent.replay_buffer.sample(batch_idx)
        ...
        if (total_step + 1) % TARGET_UPDATE_INTERVAL == 0:
            ...
            for agent_i in range(NUM_AGENT):
                ...
                # target critic 
                critic_target_q = agent.target_critic.forward(batch_next_states_tensor, multi_batch_next_actions_tensor.detach()) 
                ...
                multi_batch_online_actions_list = [single_batch_online_action if j == agent_i else single_batch_online_action.detach() for j, single_batch_online_action in enumerate(multi_batch_online_actions)]

                multi_batch_online_actions_tensor = torch.cat(multi_batch_online_actions_list, dim=1).to(device)
                # Update actor 
                actor_loss = agent.critic.forward(batch_states_tensor,multi_batch_online_actions_tensor).flatten()
    ...
    print(f"{episode_i} Episode reward: {episode_reward}")

    # 3 Render the env
    if (episode_i + 1) % 200 == 0:
        env = simple_adversary_v3.parallel_env(N=2, max_cycles=NUM_STEP, continuous_actions=True, render_mode="human")

        for test_epi_i in range(2):
            muti_obs, infos = env.reset()
            for test_step_i in range(NUM_STEP):
                multi_actions = {}
                for agent_i, agent_name in enumerate(agent_name_list):
                    agent = agents[agent_i]
                    single_obs = multi_obs[agent_name]
